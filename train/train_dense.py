...
class DataArguments:
    ...
    eval_data_file: Optional[str] = field(default=None, metadata={"help": "Path to the validation data."})
...
class SFTDataset(Dataset):
    ...
    def load_data(self, data_file):
        if data_file is None:
            raise ValueError("data_file must be provided unless --use_dummy_data is set.")
    ...
    def encode_data(self, data_dict):
        ...
        if "messages" not in data_dict:
            raise ValueError("Each JSONL row must contain a 'messages' field.")
        ...
        # The 7B model uses a different vocabulary/special tokens than smaller dense models.
...
def make_supervised_data_module(tokenizer, data_args) -> Dict:
    ...
    if data_args.use_dummy_data:
        ...
        eval_dataset = DummyDataset(tokenizer, data_args.max_seq_length) if data_args.eval_data_file else None
    else:
        ...
        if data_args.eval_data_file:
            eval_dataset = SFTDataset(
                tokenizer=tokenizer,
                data_file=data_args.eval_data_file,
                max_seq_length=data_args.max_seq_length,
                model_size=data_args.model_size,
            )
...
def train():
    sys.argv = ["--adapter_path" if arg == "--adapter-path" else arg for arg in sys.argv]
    ...
    if model_args.adapter_path and not model_args.use_lora:
        raise ValueError("--adapter_path requires --use_lora.")

    if model_args.use_lora:
        if model_args.adapter_path:
            if not os.path.exists(model_args.adapter_path):
                raise FileNotFoundError(f"Adapter path {model_args.adapter_path} does not exist.")
            print(f"Loading trainable LoRA adapter from: {model_args.adapter_path}")
    ...
    has_eval = data_module["eval_dataset"] is not None
    if has_eval and getattr(training_args, "eval_strategy", "no") == "no":
        training_args.eval_strategy = "steps"
    if not has_eval and training_args.load_best_model_at_end:
        raise ValueError("--load_best_model_at_end requires --eval_data_file.")
    ...
    if has_eval and training_args.early_stopping_patience > 0 and training_args.load_best_model_at_end:
